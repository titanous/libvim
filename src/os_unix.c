...
#define BLOCK_SIGNALS(set) \
  do                       \
  { /**/                   \
  } while (0)
#define UNBLOCK_SIGNALS(set) \
  do                         \
  { /**/                     \
  } while (0)
#endif
...
mch_total_mem(int special UNUSED)
{
...
#ifdef MACOS_X
  {
    ...
    if (kret == KERN_SUCCESS)
      ...
      mem = (long_u)(vm_stat.free_count + vm_stat.active_count + vm_stat.inactive_count
...
#if defined(HAVE_SYS_RESOURCE_H) && defined(HAVE_GETRLIMIT)
  {
    ...
    if (getrlimit(RLIMIT_DATA, &rlp) == 0 && rlp.rlim_cur < ((rlim_t)1 << (sizeof(long_u) * 8 - 1))
#ifdef RLIM_INFINITY
...
get_stack_limit(void)
{
  ...
  if (getrlimit(RLIMIT_STACK, &rlp) == 0 && rlp.rlim_cur < ((rlim_t)1 << (sizeof(long_u) * 8 - 1))
#ifdef RLIM_INFINITY
      && rlp.rlim_cur != RLIM_INFINITY
#endif
  )
 ...
 */
int mch_check_win(int argc UNUSED, char **argv UNUSED)
{
  if (isatty(1))
    return OK;
  return FAIL;
...
#if defined(ELAPSED_TIMEVAL) && (defined(FEAT_XCLIPBOARD))
  ...
  if (mch_lstat((char *)name, &st) >= 0)
  {
    ...
    if (dirp != NULL)
    {
      while ((dp = readdir(dirp)) != NULL)
      {
        ...
        if (STRICMP(tail, dp->d_name) == 0 && STRLEN(tail) == STRLEN(dp->d_name))
        {
          ...
          vim_strncpy(newname + (tail - name), (char_u *)dp->d_name,
                      MAXPATHL - (tail - name));
          if (mch_lstat((char *)newname, &st2) >= 0 && st.st_ino == st2.st_ino && st.st_dev == st2.st_dev)
          {
 ...
 */
void mch_set_acl(char_u *fname UNUSED, vim_acl_T aclent)
{
  if (aclent == NULL)
    return;
#ifdef HAVE_POSIX_ACL
 ...
 */
void mch_hide(char_u *name UNUSED)
{
  /* can't hide a file */
}
 ...
 */
void may_send_sigint(int c UNUSED, pid_t pid UNUSED, pid_t wpid UNUSED)
{
#ifdef SIGINT
    ...
    int options) /* SHELL_*, see vim.h */
{
  ...
  else
  {
    ...
    if (ofn)
      ...
      x = system((char *)cmd);
#else
    newcmd = alloc(STRLEN(p_sh) + (extra_shell_arg == NULL ? 0 : STRLEN(extra_shell_arg)) + STRLEN(p_shcf) + STRLEN(cmd) + 4);
    if (newcmd == NULL)
      x = 0;
    else
    {
...
void mch_job_start(char **argv, job_T *job, jobopt_T *options, int is_terminal)
{
  ...
  if (options->jo_pty && (!(use_file_for_in || use_null_for_in) || !(use_file_for_out || use_null_for_out) || !(use_out_for_err || use_file_for_err || use_null_for_err)))
  ...
  if (use_file_for_in)
  {
	 ...
	 * the pipe can be closed when all lines were written. */
      if (!use_null_for_in && (pty_master_fd < 0 || use_buffer_for_in) && pipe(fd_in) < 0)
    goto failed;
...
{
  if ((curr_tmode == TMODE_RAW || force) && RealWaitForChar(read_cmd_fd, 0L, NULL, NULL))
...
#ifdef FEAT_XCLIPBOARD
    if (ret > 0 && xterm_Shell != (Widget)0 && FD_ISSET(ConnectionNumber(xterm_dpy), &rfds))
    {
      xterm_update(); /* Maybe we should hand out clipboard */
    ...
    int *number_result)
{
  ...
  if (hinstLib != NULL)
  {
...
#endif
    {
      ...
      else if (retval_str != NULL && retval_str != (char_u *)1 && retval_str != (char_u *)-1)
